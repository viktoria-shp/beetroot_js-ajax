...
const getMovies = async params => {
  try {
    ...
    const res = await fetch(`${_http.urls.search}&s=${params.title}&type=${params.type}&page=${params.page}`);
...
const getMovieById = async movie_id => {
  try {
    const res = await fetch(`${_http.urls.search}&i=${movie_id}`);
    const parsedRes = await res.json();
    return parsedRes;
  } catch (error) {
    _uikit.default.notify({
...
const addMovieToFavourite = (id, el) => {
  if (!id) return;
  let favorites = getFavouritesItems();
  const index = favorites.indexOf(id); // item is not favorite

  if (index == -1) {
    favorites.push(id);
    el.classList.add("active_add"); // item is already favorite
  } else {
